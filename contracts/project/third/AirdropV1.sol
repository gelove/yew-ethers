...
contract MultiSenderV1 is Storage, Ownable {
    ...
    function registerVIP(address[] memory beneficiarys, uint256 duration) public onlyOwner {
        ...
        for (uint256 i = 0; i < beneficiarys.length; i++) {
            if (vipMap[beneficiarys[i]].startedAt == 0) {
                ...
                vipMap[beneficiarys[i]].expiredAt = vipMap[beneficiarys[i]].expiredAt + duration;
            }
        }
    }

    function buyVIP(bytes32 skuId, address inviter, bytes32 orderid, address beneficiary) public payable {
        IFinance finance = IFinance(checkoutContract);
        ProductItem memory productInfo = finance.queryProduct(skuId);
        require(
            productInfo.appId == 0x6d756c746973656e646572000000000000000000000000000000000000000000,
        ...
        if (vipMap[beneficiary].startedAt == 0) {
            ...
            vipMap[beneficiary].expiredAt = block.timestamp + productInfo.duration;
        } else {
            vipMap[beneficiary].expiredAt = vipMap[beneficiary].expiredAt + productInfo.duration;
        }
    }
    ...
    function multisendToken(
        ...
        if (vipMap[msg.sender].expiredAt < block.timestamp) {
            ...
            finance.checkout{value: msg.value - minMainCoin}(orderid, msg.sender, skuId, inviter);
        } else {
            //   VIP send for free
            if (address(0) == token) {
                require(
        ...
        if (address(0) == token) {
            //solhint-disable reason-string
            require(address(msg.sender).balance >= total, "MultiSenderV1: insufficient MainCoin balance");
            // Main Coin multisend
    ...
    function queryVIP(address target) public view returns (VIPStats memory vipStats) {
    ...
    function withdrawERC20(address token) public onlyOwner {
        ...
        erc20Token.transfer(address(owner()), erc20Token.balanceOf(address(this)));
